...
// Path B: YouTube audio is now handled by the offscreen tabCapture engine
// (works on live/SABR/DRM where per-element Web Audio can't). When true, the
// content script does NOT engage Web Audio on YouTube — the element plays
// normally to the tab output, where tabCapture grabs and boosts it.
const YT_TABCAPTURE = true;

function ensureContext() {
  if (!IS_YOUTUBE || YT_TABCAPTURE) return null;
...
// A new AudioContext starts "suspended" under Chrome's autoplay policy, and
// the per-element "play" listener misses videos that were already playing
// before we hooked them. Calling resume() in response to a user-initiated
// control (the popup) reliably wakes the graph, because by then the page has
// user activation. Without this, the rerouted audio is silent and every
...
function makeSourceForElement(el, ctx) {
  ...
  //
  // Prefer createMediaElementSource: it REROUTES the element's audio through
  // our graph, so the gain/EQ/mode nodes fully control what you hear. This is
  // the correct primary path (YouTube uses same-origin blob: MSE, so Web Audio
  // can access it without tainting).
  //
  // captureStream is only a fallback: it taps a *copy* of the audio while the
  // element keeps playing through the normal output, so the gain node only
  // affects the copy — which made volume/mute/EQ appear to do nothing.
  try {
    return ctx.createMediaElementSource(el);
  } catch (e) {
    // Element is already bound to a context (e.g. re-hook after cleanup),
    // or not permitted. Fall back to a captured copy.
  }
