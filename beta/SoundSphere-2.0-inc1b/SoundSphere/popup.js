...
let fxState = { width: 100, clarity: 0, night: false }; // width in %, clarity dB
  ...
  if (!captureStarted) {
    ...
    chrome.runtime.sendMessage(
      { type: "ss-start-capture", tabId: currentTabId, volume: value,
        ...
        fx: { width: fxState.width / 100, clarity: fxState.clarity, night: fxState.night } },
      resp => {
    ...
    chrome.runtime.sendMessage({ type: "ss-set-volume", volume: value }, resp => {
      ...
      if (resp && resp.needsStart) rehookCapture();
    });
  }
}

// Re-establish capture (carrying current mode, EQ, and enhancements) if the
// offscreen graph was torn down; a fresh start applies them from its payload.
function rehookCapture() {
  ssLog("control hit dead capture; re-hooking");
  captureStarted = false;
  captureRetries = 0;
  applyEngineVolume(Number(slider && slider.value) || 100);
}
...
function setMode(mode) {
  ...
  storageSet({ mode });
  chrome.runtime.sendMessage({ type: "ss-set-mode", mode }, resp => {
    if (chrome.runtime.lastError) return;
    if (resp && resp.needsStart) rehookCapture();
  });
}
...
function sendEqToContent() {
  chrome.runtime.sendMessage({ type: "ss-set-eq", gains: eqGains }, resp => {
    if (chrome.runtime.lastError) return;
    if (resp && resp.needsStart) rehookCapture();
  });
}
...
async function loadPrefsAndTab() {
  const data = await storageGet({
    ...
    vol_global: 100,
    vol_hosts: [],
    fx: { width: 100, clarity: 0, night: false }
  });

  if (data.fx && typeof data.fx === "object") {
    fxState = Object.assign({ width: 100, clarity: 0, night: false }, data.fx);
  }
...
function initFxControls() {
  const widthEl = document.getElementById("fxWidth");
  const clarityEl = document.getElementById("fxClarity");
  const nightEl = document.getElementById("fxNight");

  if (widthEl) {
    widthEl.value = String(fxState.width);
    widthEl.addEventListener("input", () => {
      fxState.width = Number(widthEl.value) || 100;
      sendFx();
    });
    widthEl.addEventListener("change", () => storageSet({ fx: fxState }));
  }
  if (clarityEl) {
    clarityEl.value = String(fxState.clarity);
    clarityEl.addEventListener("input", () => {
      fxState.clarity = Number(clarityEl.value) || 0;
      sendFx();
    });
    clarityEl.addEventListener("change", () => storageSet({ fx: fxState }));
  }
  if (nightEl) {
    const render = () => {
      nightEl.textContent = fxState.night ? "On" : "Off";
      nightEl.classList.toggle("on", fxState.night);
      nightEl.setAttribute("aria-pressed", String(fxState.night));
    };
    render();
    nightEl.addEventListener("click", () => {
      fxState.night = !fxState.night;
      render();
      sendFx();
      storageSet({ fx: fxState });
    });
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  loadPrefsAndTab()
    .then(() => initFxControls())
    .catch(e => ssLog("loadPrefsAndTab ERROR:", e && e.message));
