...
async function startCaptureForTab(tabId, volume, mode, gains, fx) {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === "ss-start-capture") {
    startCaptureForTab(msg.tabId, msg.volume, msg.mode, msg.gains, msg.fx)
  ...
  if (msg.type === "ss-set-fx") {
    chrome.runtime.sendMessage({ target: "offscreen", type: "fx", fx: msg.fx })
      .then(resp => sendResponse(resp || { ok: true }))
      .catch(() => sendResponse({ ok: false, needsStart: true }));
    return true; // async
  }

  if (msg.type === "ss-spectrum") {
    chrome.runtime.sendMessage({ target: "offscreen", type: "spectrum" })
